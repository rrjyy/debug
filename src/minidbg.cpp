...
void debugger::initialise_load_address()
{
    ...
    if (m_elf.get_hdr().type == elf::et::dyn)
    {
        ...
        m_load_address = std::stoull(addr, 0, 16); // stoi may out of range of int
...
void debugger::handle_sigtrap(siginfo_t info)
{
    switch (info.si_code)
    {
        ...
        case TRAP_BRKPT:
        {
            ...
            auto offset_pc = offset_load_address(get_pc()); //for querying DWARF
...
{
    for (const auto& rd : g_register_descriptots)
    {
        std::cout << rd.name << " 0x"
